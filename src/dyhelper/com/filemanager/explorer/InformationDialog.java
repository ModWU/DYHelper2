...
public class InformationDialog extends AlertDialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        mView = getLayoutInflater().inflate(R.layout.filemanager_information_dialog, null);

        if (mFileInfo.IsDir) {
            setIcon(R.drawable.filemanager_folder);
            asyncGetSize();
        } else {
            setIcon(R.drawable.filemanager_file_icon_default);
        }
        ...
        ((TextView) mView.findViewById(R.id.information_canread))
                .setText(mFileInfo.canRead ? R.string.filemanager_yes : R.string.filemanager_no);
        ((TextView) mView.findViewById(R.id.information_canwrite))
                .setText(mFileInfo.canWrite ? R.string.filemanager_yes : R.string.filemanager_no);
        ((TextView) mView.findViewById(R.id.information_ishidden))
                .setText(mFileInfo.isHidden ? R.string.filemanager_yes : R.string.filemanager_no);

        setView(mView);
        setButton(BUTTON_NEGATIVE, mContext.getString(R.string.filemanager_confirm_know), (DialogInterface.OnClickListener) null);
    ...
    private void onSize(final long size) {
        ...
        msg.setData(bd);
        mHandler.sendMessage(msg); // 鍚慔andler鍙戦�佹秷鎭�,鏇存柊UI
    }
    ...
    private String formatFileSizeString(long size) {
        ...
        if (size >= 1024) {
            ...
            ret += (" (" + mContext.getResources().getString(R.string.filemanager_file_size, size) + ")");
        } else {
            ret = mContext.getResources().getString(R.string.filemanager_file_size, size);
        }
